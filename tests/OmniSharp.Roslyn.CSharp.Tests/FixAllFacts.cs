...
public class FixAllFacts
{
    ...
    [Fact]
    public async Task WhenFileContainsFixableIssuesWithAnalyzersEnabledThenFixThemAllAsync()
    {
        using OmniSharpTestHost host = GetHost(true);
        string originalText = @"class C {}";
        string expectedText = @"internal class C { }";
        string testFilePath = CreateTestProjectWithDocument(host, originalText);
        string textBeforeFix = await GetContentOfDocumentFromWorkspace(host, testFilePath).ConfigureAwait(true);
        RunFixAllCodeActionService handler = host.GetRequestHandler<RunFixAllCodeActionService>(OmniSharpEndpoints.RunFixAll);
        RunFixAllResponse response = await handler.Handle(new RunFixAllRequest
        (
        ...
        string textAfterFix = await GetContentOfDocumentFromWorkspace(host, testFilePath).ConfigureAwait(true);
        AssertUtils.AssertIgnoringIndent(expectedText, textAfterFix);
        LinePositionSpanTextChange internalClassChange = response.Changes.OfType<ModifiedFileResponse>().Single().Changes.Single(x => x.NewText is "internal ");
        ...
        LinePositionSpanTextChange formatFix = response.Changes.OfType<ModifiedFileResponse>().Single().Changes.Single(x => x.NewText is " ");
    ...
    [Fact]
    public async Task WhenFixAllItemsAreDefinedByFilterThenFixOnlyFilteredItems()
    {
        using OmniSharpTestHost host = GetHost(true);
        string originalText = "    class C{}";
    ...
    public async Task WhenFixAllIsScopedToDocumentAndProjectThenOnlyFixInScopeInsteadOfEverythingDifferentProjects(FixAllScope scope)
    {
        using OmniSharpTestHost host = GetHost(true);
        string originalIde0055Text =
        @"
            internal class InvalidFormatIDE0055ExpectedHere{}
        ";

        string expectedIde0055TextWithFixedFormat =
        @"
            internal class InvalidFormatIDE0055ExpectedHere { }
        ";

        string fileInScope = CreateTestProjectWithDocument(host, originalIde0055Text);

        string fileNotInScope = CreateTestProjectWithDocument(host, originalIde0055Text);
        ...
        string textAfterFixInScope = await GetContentOfDocumentFromWorkspace(host, fileInScope).ConfigureAwait(true);
        string textAfterNotInScope = await GetContentOfDocumentFromWorkspace(host, fileNotInScope).ConfigureAwait(true);
    ...
    [Fact]
    public async Task WhenFixAllIsScopedToDocumentAndProjectThenOnlyFixInScopeInsteadOfEverythingDifferentDocumentsAsync()
    {
...
";
        ...
        using OmniSharpTestHost host = GetHost(true);
        IEnumerable<ProjectId> projectId = host.AddFilesToWorkspace(new TestFile(File1, UnformattedString), new TestFile(File2, UnformattedString)).First();
...
private readonly object o = 1;
    ...
    public async Task WhenAvailableFixAllActionsAreRequested_ThenReturnThemAtResponse()
    {
        using (var host = GetHost(true))
        {
            ...
            Assert.Contains(resultFromDocument.Items, x => x.Id is "IDE0055");
            Assert.DoesNotContain(resultFromDocument.Items, x => x.Id is "IDE0040");
        }
    }
