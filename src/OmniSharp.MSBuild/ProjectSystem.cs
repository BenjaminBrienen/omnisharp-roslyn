...
internal class ProjectSystem : IProjectSystem, IDisposable
{
    ...
    public void Initalize(IConfiguration configuration)
    {
        ...
        {
            string buildEnvironmentInfo = MSBuildHelpers.GetBuildEnvironmentInfo();
        ...
        IEnumerable<(string, ProjectIdInfo)> initialProjectPathsAndIds = GetInitialProjectPathsAndIds();

        foreach ((string projectFilePath, ProjectIdInfo projectIdInfo) in initialProjectPathsAndIds)
        {
    ...
    public Task WaitForIdleAsync() => _manager.WaitForQueueEmptyAsync();
    ...
    private IEnumerable<(string, ProjectIdInfo)> GetProjectPathsAndIdsFromSolutionOrFilter(string solutionOrFilterFilePath)
    {
        ...
        foreach (var globalSection in solutionFile.GlobalSections)
        {
            ...
            if (globalSection.Name is "ProjectConfigurationPlatforms")
            {
