...
public class OmniSharpWorkspace : Workspace
{
    ...
    private readonly ConcurrentBag<Func<string, Task>> _waitForProjectModelReadyHandlers = new();
    private readonly ConcurrentDictionary<string, ProjectInfo> miscDocumentsProjectInfos = new();
    private readonly ConcurrentDictionary<ProjectId, Predicate<string>> documentInclusionRulesPerProject = new();
    ...
    private void OnDirectoryRemoved(string path, FileChangeType changeType)
    {
        ...
        {
            IEnumerable<Document> docs = CurrentSolution.Projects.SelectMany(x => x.Documents)
    ...
    public void AddWaitForProjectModelReadyHandler(Func<string, Task> handler) => _waitForProjectModelReadyHandlers.Add(handler);

    public override void OpenDocument(DocumentId documentId, bool activate = true)
    {
        Document? doc = CurrentSolution.GetDocument(documentId);
        if (doc is not null)
        {
            SourceText text = doc.GetTextAsync(CancellationToken.None).WaitAndGetResult(CancellationToken.None);
    ...
    public override void CloseDocument(DocumentId documentId)
    {
        ...
        {
            SourceText text = doc.GetTextAsync(CancellationToken.None).WaitAndGetResult(CancellationToken.None);
            VersionStamp version = doc.GetTextVersionAsync(CancellationToken.None).WaitAndGetResult(CancellationToken.None);
            var loader = TextLoader.From(TextAndVersion.Create(text, version, doc.FilePath));
            OnDocumentClosed(documentId, loader);
        }
    }

    public void AddProject(ProjectInfo projectInfo) => OnProjectAdded(projectInfo);

    public void AddDocumentInclusionRuleForProject(ProjectId projectId, Predicate<string> documentPathFilter) => documentInclusionRulesPerProject[projectId] = documentPathFilter;

    public void AddProjectReference(ProjectId projectId, ProjectReference projectReference) => OnProjectReferenceAdded(projectId, projectReference);

    public void RemoveProjectReference(ProjectId projectId, ProjectReference projectReference) => OnProjectReferenceRemoved(projectId, projectReference);

    public void AddMetadataReference(ProjectId projectId, MetadataReference metadataReference) => OnMetadataReferenceAdded(projectId, metadataReference);

    public void RemoveMetadataReference(ProjectId projectId, MetadataReference metadataReference) => OnMetadataReferenceRemoved(projectId, metadataReference);

    public DocumentId TryAddMiscellaneousDocument(string filePath, TextLoader loader, string language)
    {
        if (GetDocument(filePath) is not null)
            return null; //if the workspace already knows about this document then it is not a miscellaneous document

        ProjectInfo projectInfo = miscDocumentsProjectInfos.GetOrAdd(language, (lang) => CreateMiscFilesProject(lang));
        DocumentId documentId = AddDocument(projectInfo.Id, filePath, loader);
        _logger.LogInformation($"Miscellaneous file: {filePath} added to workspace");

        if (!EditorConfigEnabled)
            return documentId;

        IEnumerable<string?> analyzerConfigFiles = projectInfo.AnalyzerConfigDocuments.Select(document => document.FilePath);
        IEnumerable<string?> newAnalyzerConfigFiles = EditorConfigFinder
            .GetEditorConfigPaths(filePath)
            .Except(analyzerConfigFiles);

        foreach (string? analyzerConfigFile in newAnalyzerConfigFiles)
        {
    ...
    public DocumentId TryAddMiscellaneousDocument(string filePath, string language) => TryAddMiscellaneousDocument(filePath, new OmniSharpTextLoader(filePath), language);

    public bool TryRemoveMiscellaneousDocument(string filePath)
    {
        DocumentId documentId = GetDocumentId(filePath);
        if (documentId is null || !IsMiscellaneousDocument(documentId))
            return false;
    ...
    public void TryPromoteMiscellaneousDocumentsToProject(Project project)
    {
        ...
        ProjectInfo[] miscProjectInfos = miscDocumentsProjectInfos.Values.ToArray();
        for (int i = 0; i < miscProjectInfos.Length; i++)
        {
            Project? miscProject = CurrentSolution.GetProject(miscProjectInfos[i].Id);
            Document[] documents = miscProject.Documents.ToArray();

            for (int j = 0; j < documents.Length; j++)
            {
                Document document = documents[j];
    ...
    public void UpdateCompilationOptionsForProject(ProjectId projectId, CompilationOptions options) => OnCompilationOptionsChanged(projectId, options);
    ...
    public DocumentId AddDocument(DocumentId documentId, Project project, string filePath, SourceCodeKind sourceCodeKind = SourceCodeKind.Regular) => AddDocument(documentId, project, filePath, new OmniSharpTextLoader(filePath), sourceCodeKind);
    ...
    {
        string? basePath = Path.GetDirectoryName(project.FilePath);
        ...
        try
        {
            ...
            string relativeDocumentPath = FileSystemHelper.GetRelativePath(fullPath, basePath);

            // only set document's folders if
            // 1. relative path was computed
            // 2. path is not pointing any level up
            if (relativeDocumentPath is not null && !relativeDocumentPath.StartsWith(".."))
    ...
    public void RemoveDocument(DocumentId documentId) => OnDocumentRemoved(documentId);

    public void RemoveProject(ProjectId projectId) => OnProjectRemoved(projectId);

    public void SetCompilationOptions(ProjectId projectId, CompilationOptions options) => OnCompilationOptionsChanged(projectId, options);

    public void SetParseOptions(ProjectId projectId, ParseOptions parseOptions) => OnParseOptionsChanged(projectId, parseOptions);

    public void OnDocumentChanged(DocumentId documentId, SourceText text) => OnDocumentTextChanged(documentId, text, PreservationMode.PreserveIdentity);

    public DocumentId GetDocumentId(string filePath)
    {
        ImmutableArray<DocumentId> documentIds = CurrentSolution.GetDocumentIdsWithFilePath(filePath);
    ...
    internal bool FileBelongsToProject(string fileName, Project project)
    {
        ...
        DirectoryInfo? fileDirectory = new FileInfo(fileName).Directory;
        ...
        while (fileDirectory is not null)
        {
    ...
    private void LogDeletionException(Exception e, string filePath) => _logger.LogError(e, $"Error deleting file {filePath}");
    ...
    protected override void ApplyDocumentAdded(DocumentInfo info, SourceText text)
    {
        ...
        if (text is not null)
            SaveDocumentText(info.Id, fullPath, text, text.Encoding ?? Encoding.UTF8);
    }
    ...
    public bool IsCapableOfSemanticDiagnostics(Document document) => !IsMiscellaneousDocument(document.Id);

    private bool IsMiscellaneousDocument(DocumentId documentId) => miscDocumentsProjectInfos.Any(p => p.Value.Id == documentId.ProjectId);

    private class DelegatingTextLoader : TextLoader
    {
        private readonly Document _fromDocument;

        public DelegatingTextLoader(Document fromDocument) => _fromDocument = fromDocument ?? throw new ArgumentNullException(nameof(fromDocument));
        ...
        {
            SourceText sourceText = await _fromDocument.GetTextAsync();
            VersionStamp version = await _fromDocument.GetTextVersionAsync();
    ...
    private Task OnWaitForProjectModelReadyAsync(string filePath) => Task.WhenAll(_waitForProjectModelReadyHandlers.Select(h => h(filePath)));
    ...
    public void SetAnalyzerReferences(ProjectId id, ImmutableArray<AnalyzerFileReference> analyzerReferences)
    {
        ...
        IEnumerable<AnalyzerFileReference> refsToAdd = analyzerReferences.Where(newRef => project.AnalyzerReferences.All(oldRef => oldRef.Display != newRef.Display));
        IEnumerable<AnalyzerReference> refsToRemove = project.AnalyzerReferences.Where(newRef => analyzerReferences.All(oldRef => oldRef.Display != newRef.Display));

        foreach (AnalyzerFileReference? toAdd in refsToAdd)
        {
            _logger.LogInformation($"Adding analyzer reference: {toAdd.FullPath}");
            OnAnalyzerReferenceAdded(id, toAdd);
        }

        foreach (AnalyzerReference? toRemove in refsToRemove)
        {
            _logger.LogInformation($"Removing analyzer reference: {toRemove.FullPath}");
            OnAnalyzerReferenceRemoved(id, toRemove);
        }
    }
    ...
    public void RemoveAdditionalDocument(DocumentId documentId) => OnAdditionalDocumentRemoved(documentId);

    public void RemoveAnalyzerConfigDocument(DocumentId documentId) => OnAnalyzerConfigDocumentRemoved(documentId);
    ...
    protected override void ApplyProjectChanges(ProjectChanges projectChanges)
    {
        ...
        {
            MethodInfo? onDefaultNamespaceChanged = GetType().GetMethod("OnDefaultNamespaceChanged", BindingFlags.Instance | BindingFlags.NonPublic);
            if (onDefaultNamespaceChanged is not null)
